  ...
  protected void setPixels() {
    ...
    if (this.lastDispose > 0) {
      if (this.lastDispose == 3) {
        int j = this.frameCount - 2;
        if (j > 0) {
          this.lastImage = getFrame(j - 1);
        } else {
          this.lastImage = null;
        } 
      } 
    ...
    for (int i = 0; i < this.ih; i++) {
      ...
      if (line < this.height) {
        ...
        int var10 = i * this.iw;
        while (dx < dlim) {
          int index = this.pixels[var10++] & 0xFF;
  ...
  protected void decodeImageData() {
    ...
    while (i < npix) {
      if (top == 0) {
        ...
        int j = datum & code_mask;
        datum >>= code_size;
        bits -= code_size;
        if (j > available)
          break; 
        if (j == end_of_information)
          break; 
        if (j == clear) {
        ...
        if (old_code == -1) {
          this.pixelStack[top++] = this.suffix[j];
          old_code = j;
          first = j;
          continue;
        } 
        int in_code;
        if ((in_code = j) == available) {
