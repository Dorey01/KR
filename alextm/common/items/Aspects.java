...
public class Aspects extends Item {
  public Aspects(String name, String texture, int j) {
    func_77655_b(name);
  ...
  public void func_77624_a(ItemStack itemStack, EntityPlayer player, List<String> list, boolean isAdv) {
    list.add(SkyHard.resource("Aspects.tooltip_1"));
    list.add(SkyHard.resource("Aspects.tooltip_2"));
    list.add(SkyHard.resource("Aspects.tooltip_3"));
    list.add(SkyHard.resource("Aspects.tooltip_4"));
  }
  
  public ItemStack func_77659_a(ItemStack stack, World world, EntityPlayer player) {
    if (!world.field_72995_K) {
      MinecraftServer server = FMLCommonHandler.instance().getMinecraftServerInstance();
      String command = "tc aspect $nick all 100".replace("$nick", player.func_70005_c_());
