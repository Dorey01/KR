...
public class EndPortal extends Item {
  public EndPortal(String name, String texture, int j) {
    func_77655_b(name);
  ...
  public void func_77624_a(ItemStack itemStack, EntityPlayer player, List<String> list, boolean isAdv) {
    list.add(SkyHard.resource("EndPortal.tooltip_1"));
    list.add(SkyHard.resource("EndPortal.tooltip_2"));
    list.add(SkyHard.resource("EndPortal.tooltip_3"));
    list.add(SkyHard.resource("EndPortal.tooltip_4"));
  }
  
  public ItemStack func_77659_a(ItemStack stack, World world, EntityPlayer player) {
    if (!world.field_72995_K) {
      MinecraftServer server = FMLCommonHandler.instance().getMinecraftServerInstance();
      String command = "rg addmember end $nick -w DIM1".replace("$nick", player.func_70005_c_());
